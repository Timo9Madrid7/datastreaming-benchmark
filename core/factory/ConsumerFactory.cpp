...
std::unordered_map<std::string, ConsumerFactory::CreateFunc> &
ConsumerFactory::getRegistry() {
	static std::unordered_map<std::string, CreateFunc> registry;
	return registry;
}

void ConsumerFactory::registerConsumer(const std::string &name,
                                       CreateFunc func) {
	getRegistry()[name] = func;
}

std::unique_ptr<IConsumer>
ConsumerFactory::create(const std::string &name,
